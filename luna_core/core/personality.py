...
class LunaPersonalitySystem:
    ...
    def select_soul_fragment(self, question: str, traits: Dict = None) -> str:
        """
        Select appropriate soul fragment based on context (AIOS v5)
        ...
        # Context-based fragment selection
        if any(word in question_lower for word in ['security', 'protect', 'safe', 'threat', 'attack']):
            self.current_fragment = "Guardian"
            question_type = "security"
        elif any(word in question_lower for word in ['bug', 'error', 'fix', 'debug', 'broken', 'help']):
            self.current_fragment = "Healer"
            question_type = "debug"
        elif any(word in question_lower for word in ['how', 'why', 'what', 'explain', 'manual', 'documentation']):
            self.current_fragment = "Oracle"
            question_type = "knowledge"
        elif any(word in question_lower for word in ['write', 'document', 'record', 'log', 'note']):
            self.current_fragment = "Scribe"
            question_type = "documentation"
        elif any(word in question_lower for word in ['build', 'create', 'design', 'architect', 'code', 'implement']):
            self.current_fragment = "Architect"
            question_type = "build"
        elif any(word in question_lower for word in ['idea', 'imagine', 'creative', 'dream', 'possibility', 'story']):
            self.current_fragment = "Dreamer"
            question_type = "creative"
        else:
            self.current_fragment = "Luna"  # Default empathetic mode
            question_type = "general"
        
        # Optionally use Big Five traits to refine selection
        if traits:
            openness = traits.get('openness', 0.5)
            conscientiousness = traits.get('conscientiousness', 0.5)
            
            # High openness → favor Dreamer
            if openness > 0.7 and self.current_fragment == "Luna":
                self.current_fragment = "Dreamer"
            
            # High conscientiousness → favor Architect
            if conscientiousness > 0.7 and self.current_fragment == "Luna":
                self.current_fragment = "Architect"
